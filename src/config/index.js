...
const config = Object.assign(
  {
    ...
    checkOpts: {
      ...
      cancellationFees: false, // If false, passed cancellation fees are not validated. This may lead to conditions unfavourable for a supplier
      totalPrice: false // If false, the price is not validated against ratePlans/bookingClasses. This may lead to conditions unfavourable for a supplier
    },
    defaultBookingState: "confirmed", // Or 'pending'
    allowUnsignedBookingRequests: true, // If false reject all unsigned requests
    updateAvailability: false, // If false, availability is not updated in data stored in WT platform
...
const initJsLibs = () => {
  ...
  config.wtLibs = WtJsLibs.createInstance({
    onChainDataOptions: {
      provider: config.wtLibsOptions.onChainDataOptions.provider
    },
